...
class FeeRepositoryImpl(
    ...
    // FIX: remoteDataSource.getPaymentsByStudent ahora exige
    // institutionId (antes no filtraba por institución). Se resuelve
    // a partir del propio estudiante.
    override fun getPaymentsByStudent(studentId: Long): Flow<Resource<List<FeePayment>>> {
        return networkBoundResource(
            query = { localDataSource.getPaymentsByStudent(studentId) },
            fetch = {
                val institutionId = studentRepository.getStudentById(studentId)?.institutionId ?: ""
                remoteDataSource.getPaymentsByStudent(studentId, institutionId)
            },
    ...
    override suspend fun registerPayment(payment: FeePayment): Resource<Boolean> {
        return try {
            ...
            val success = withContext(Dispatchers.Default) {
                try {
                    val student = studentRepository.getStudentById(payment.studentId)
                    if (student != null) {
                        val pdfBytes = receiptGenerator.generateReceiptPdf(payment, student)
                        val receiptUrl = remoteDataSource.uploadReceipt(payment.id, pdfBytes)
    ...
    override suspend fun getReceiptUrl(receiptPath: String): String? {
        return remoteDataSource.getReceiptSignedUrl(receiptPath)
    }
